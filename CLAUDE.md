...
## 実機で出た不具合（v4.2.2・2026-07-25）
- **起動直後がブランクページになる**（池本が実機で遭遇）。真因は v4.2.0 で入れたクリップボード引き継ぎの`handoff.normalize()`が緩すぎたこと：`javascript:...`（＝ワンクリック設定で**自前でクリップボードに入れたブックマークレット**）や`ver.4.2.1をリリースした`のようなメモまで`https://`を前置してURL扱い→到達不能→`except: pass`で**about:blankのまま**。
  - 対策1: `normalize()`を厳格化（スキームはhttp/httpsのみ・他スキームは明示的に拒否／ホスト末尾ラベルはASCIIのTLD必須／空白や`'"<>{}|^\``を含むものは拒否）。
  - 対策2: **起動時に絶対にブランクで放置しない**——最初のURLが開けなければ通常の開始ページへフォールバック。失敗後のChromeは`chrome-error://chromewebdata/`になるので**これもブランク扱い**に含めること（見落とすと同じ症状が残る）。エラーページ自身の遷移で1回目のgotoが中断されるため**リトライ1回**必要。
  - 検証: `test/test_no_blank.py`（受理/拒否15件＋クリップボードに実際に書けたことを確認してから判定＝"clipboardが壊れていて素通り"の偽PASSを防ぐ）。実exeでもブックマークレットをクリップボードに入れて起動→google.comに着地を確認。
- **検証時の注意**: 実機確認で「chromeのウィンドウタイトル」を見るときは、**池本自身のChromeを拾わないよう**プロファイルパス（`*STUDIO PATTI*`）でプロセスを絞ること。また起動直後は`無題/Untitled`なので、**タイトルが確定するまで待って**から判定する（どちらも偽PASSの原因になった）。
